...
class Excel {
  ...
  private void createNewExcelFile(String fileName){
    ...
    public void make(String fileName, String sheetName, Settings settings, ObservableList<Route> observableList) {
        ...
        Integer megtettKM = sqlBuilder.getSpedometer(settings.getAktualis_honap(), settings.getRendszam());
        ...
        //**workdate->settings.getAktualis_honap()
        String fuelValue = String.valueOf(sqlBuilder.getFueling(settings.getAktualis_honap(), settings.getRendszam()));
        if (fuelValue.length() > 6) {
            fuelValue = fuelValue.substring(0, 7);
        }
        excel.setCell(fileName, sheetName, "G7", fuelValue);
        //**workdate->settings.getAktualis_honap()
        Double value = 100 * sqlBuilder.getFueling(settings.getAktualis_honap(), settings.getRendszam()) / megtettKM;
        String dValue = "";
        ...
        //**workdate->settings.getAktualis_honap()
        excel.setCell(fileName, sheetName, "L173", String.valueOf(sqlBuilder.getMaganut(settings.getAktualis_honap(), settings.getRendszam())));
        //**workdate->settings.getAktualis_honap()
        Double doubleValue = (double) sqlBuilder.getMaganut(settings.getAktualis_honap(), settings.getRendszam()) / megtettKM * 100;
